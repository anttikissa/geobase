...
test('listen with existing data', async () => {
	await db.updateObject({ type: 'a', id: 1, lat: 5, long: 5, name: 'X', v: 1 });
	await db.updateObject({ type: 'a', id: 2, lat: 15, long: 5, name: 'Y', v: 2 });
	await db.updateObject({ type: 'a', id: 3, lat: 7, long: 7, name: 'Z', v: 3 });
	...
	expect(connection.onUpdate.mock.calls[0][0]).toEqual({ type: 'a', id: 1, lat: 5, long: 5, name: 'X', v: 1 });
	expect(connection.onUpdate.mock.calls[1][0]).toEqual({ type: 'a', id: 3, lat: 7, long: 7, name: 'Z', v: 3 });
});
...
test('listen with existing data and changing properties', async () => {
	await db.updateObject({ type: 'a', id: 1, lat: 5, long: 5, name: 'X', v: 1 });
	await db.updateObject({ type: 'a', id: 2, lat: 8, long: 7, name: 'Y', v: 2 });
	await db.updateObject({ type: 'a', id: 3, lat: 11, long: 7, name: 'Z', v: 3 });
	await db.updateObject({ type: 'a', id: 4, lat: 11, long: 12, name: 'W', v: 4 });
	...
	expect(connection.onUpdate.mock.calls.length).toBe(2);
	expect(connection.onUpdate.mock.calls[0][0]).toEqual({ type: 'a', id: 1, lat: 5, long: 5, name: 'X', v: 1 });
	expect(connection.onUpdate.mock.calls[1][0]).toEqual({ type: 'a', id: 2, lat: 8, long: 7, name: 'Y', v: 2 });

	await db.listen(connection, 'a', { minLat: 6, maxLat: 12, minLong: 7, maxLong: 15 });

	expect(connection.onUpdate.mock.calls.length).toBe(4);
	expect(connection.onUpdate.mock.calls[2][0]).toEqual({ type: 'a', id: 3, lat: 11, long: 7, name: 'Z', v: 3 });
	expect(connection.onUpdate.mock.calls[3][0]).toEqual({ type: 'a', id: 4, lat: 11, long: 12, name: 'W', v: 4 });
});
...
test('getOne', async () => {
	...
	await db.updateObject({ type: 'a', id: 2, lat: 3, long: 4, v: 1 });
	expect(await db.getOne('a', 2)).toEqual({ type: 'a', id: 2, lat: 3, long: 4, v: 1 });
	await db.updateObject({ type: 'a', id: 2, lat: 3, long: 5, v: 2 });
	expect(await db.getOne('a', 2)).toEqual({ type: 'a', id: 2, lat: 3, long: 5, v: 2 });
...
test('getAll', async () => {
	await db.updateObject({ type: 'a', id: 1, lat: 5, long: 5, v: 1 });
	await db.updateObject({ type: 'a', id: 2, lat: 6, long: 6, v: 2 });
	await db.updateObject({ type: 'a', id: 3, lat: 60, long: 60, v: 3 });
	await db.updateObject({ type: 'b', id: 3, lat: 6, long: 7, v: 4 });

	expect(await db.getAll('a')).toEqual([
		{ type: 'a', id: 1, lat: 5, long: 5, v: 1 },
		{ type: 'a', id: 2, lat: 6, long: 6, v: 2 },
		{ type: 'a', id: 3, lat: 60, long: 60, v: 3 }
	]);

	expect(await db.getAll('b')).toEqual([
		{ type: 'b', id: 3, lat: 6, long: 7, v: 4 }
	]);
	...
	.toEqual([{ type: 'a', id: 1, lat: 5, long: 5, v: 1 }]);
	...
	await db.updateObject({ type: 'a', id: 3, lat: 4.7, long: 7, v: 5 });

	// Combine multiple filters
	expect(await db.getAll('a', { minLat: 4.5, maxLat: 5.5, minLong: 4.9, maxLong: 7.2  }))
	.toEqual([
		{ type: 'a', id: 1, lat: 5, long: 5, v: 1 },
		{ type: 'a', id: 3, lat: 4.7, long: 7, v: 5 }
	]);
...
test('changing only broadcasts changed properties and type and id', async () => {
	...
	await db.updateObject({ type: 'a', id: 123, lat: 5, long: 5, name: 'Hello', v: 1 });
	expect(listener.onCreate.mock.calls.length).toBe(1);
	expect(listener.onCreate.mock.calls[0][0]).toEqual({ type: 'a', id: 123, lat: 5, long: 5, name: 'Hello', v: 1 });

	// Not changing anything (even the version). Nothing should be called.
	await db.updateObject({ type: 'a', id: 123, lat: 5, long: 5, name: 'Hello', v: 1 });
	expect(listener.onCreate.mock.calls.length).toBe(1);
	expect(listener.onUpdate.mock.calls.length).toBe(0);

	// TODO figure out if this is actually the right way to do it
	// Changing just the version:
	await db.updateObject({ type: 'a', id: 123, lat: 5, long: 5, name: 'Hello', v: 2 });
	expect(listener.onUpdate.mock.calls.length).toBe(1);
	expect(listener.onUpdate.mock.calls[0][0]).toEqual({ type: 'a', id: 123, v: 2 });

	// Adding new property
	await db.updateObject({ type: 'a', id: 123, lat: 5, long: 5, name: 'Hello', orders: 123, v: 3 });
	expect(listener.onCreate.mock.calls.length).toBe(1);
	expect(listener.onUpdate.mock.calls.length).toBe(2);
	expect(listener.onUpdate.mock.calls[1][0]).toEqual({ type: 'a', id: 123, orders: 123, v: 3 });

	// Changing existing property
	await db.updateObject({ type: 'a', id: 123, lat: 5, long: 5, name: 'Hello yo', orders: 124, v: 4 });
	expect(listener.onUpdate.mock.calls.length).toBe(3);
	expect(listener.onUpdate.mock.calls[2][0]).toEqual({ type: 'a', id: 123, name: 'Hello yo', orders: 124, v: 4 });
});
...
test('changing object location', async () => {
	...
	await db.updateObject({ type: 'a', id: 123, lat: 5, long: 5, name: 'X', v: 1 });

	expect(region1Listener.onCreate.mock.calls.length).toBe(1);

	// Move object within one region.
	await db.updateObject({ type: 'a', id: 123, lat: 6, long: 5, name: 'New name', age: 20, v: 2 });

	expect(region1Listener.onUpdate.mock.calls.length).toBe(1);
	expect(region1Listener.onUpdate.mock.calls[0][0]).toEqual({
		type: 'a', id: 123, lat: 6, name: 'New name', age: 20, v: 2
	});

	// Move object from region to another, change its name at the same time
	await db.updateObject({ type: 'a', id: 123, lat: 15, long: 15, name: 'Name 2', v: 3 });

	expect(region1Listener.onUpdate.mock.calls.length).toBe(2);
	expect(region1Listener.onUpdate.mock.calls[1][0]).toEqual({
		type: 'a', id: 123, lat: 15, long: 15, name: 'Name 2', v: 3
	});
	...
	expect(region2Listener.onUpdate.mock.calls[0][0]).toEqual({
		type: 'a', id: 123, lat: 15, long: 15, name: 'Name 2', age: 20, v: 3
	});
});
...
test('different kinds of objects do not affect each other', async () => {
	...
	await db.updateObject({ type: 'a', id: 123, lat: 7, long: 7, name: 'Name A', v: 1 });
	await db.updateObject({ type: 'b', id: 123, lat: 7, long: 7, name: 'Name B', v: 2 });
	await db.updateObject({ type: 'c', id: 123, lat: 7, long: 7, name: 'Name C', v: 3 });

	expect(typeAListener.onCreate.mock.calls.length).toBe(1);
	expect(typeAListener.onCreate.mock.calls[0][0]).toEqual({ type: 'a', id: 123, lat: 7, long: 7, name: 'Name A', v: 1 });

	expect(typeBListener.onCreate.mock.calls.length).toBe(1);
	expect(typeBListener.onCreate.mock.calls[0][0]).toEqual({ type: 'b', id: 123, lat: 7, long: 7, name: 'Name B', v: 2 });

	await db.updateObject({ type: 'a', id: 123, lat: 7, long: 8, name: 'Name A', changed: 1, v: 3 });
	await db.updateObject({ type: 'b', id: 123, lat: 7, long: 9, name: 'Name B', changed: 2, v: 4 });
	await db.updateObject({ type: 'c', id: 123, lat: 7, long: 6, name: 'Name C', changed: 3, v: 5 });

	expect(typeAListener.onUpdate.mock.calls.length).toBe(1);
	expect(typeAListener.onUpdate.mock.calls[0][0]).toEqual({ type: 'a', id: 123, long: 8, changed: 1, v: 3 });
	expect(typeBListener.onUpdate.mock.calls.length).toBe(1);
	expect(typeBListener.onUpdate.mock.calls[0][0]).toEqual({ type: 'b', id: 123, long: 9, changed: 2, v: 4 });
